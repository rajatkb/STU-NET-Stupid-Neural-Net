...
def bifocal_nvidia(shape , margin):
    ...
    dense_layers = Dense(1100 , activation='relu')(dense_layers)
    dense_layers = BatchNormalization()(dense_layers)
    ...
    dense_layers = BatchNormalization()(dense_layers)
    dense_layers = Dense(10, activation='relu')(dense_layers)
    dense_layers = Dense(1)(dense_layers)
    dense_layers = Dense(10, activation='tanh')(dense_layers)
    dense_layers = BatchNormalization()(dense_layers)
    dense_layers = Dense(10, activation='tanh')(dense_layers)
    dense_layers = BatchNormalization()(dense_layers)
    dense_layers = Dense(10, activation='tanh')(dense_layers)
...
if __name__ == '__main__':
    ...
    batches=['third_batch' , 'second_batch' , 'first_batch']
    ...
    # model = bifocal_nvidia(shape , margin)
    ...
    for i in range(epoch):
        model.fit({'right_input':X[0] , 'left_input':X[1]}, Y, 
              batch_size=60,nb_epoch=1, verbose=1 , shuffle=True)
        save_model(model , "v4_nfs_iteration2_ai")

    score = model.evaluate(x={'right_input':X[0] , 'left_input':X[1]} , y=Y)
